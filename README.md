...
혼자 설계·개발·운영하고 있는 게임 정보 웹서비스의 **엔지니어링 기록**임. 집에 둔 리눅스 서버
한 대 위에서 무중단 배포와 자동 장애 복구까지 직접 굴리고 있음. **월 활성 사용자 14만 명**이
쓰는 서비스임. → [www.cookierunhub.com](https://www.cookierunhub.com)
...
> 이 저장소에는 코드와 데이터가 없음. 서비스의 콘텐츠 데이터는 직접 수집·정리한 것이라 공개하지
> 않고, 구조와 기술적 판단만 정리했음. 코드 열람이 필요하면 말씀해 주기 바람.

![세 로케일의 같은 화면](docs/images/trilingual.png)

<sub>같은 페이지, 세 로케일. 정적 문구뿐 아니라 **사용자가 쓴 조합 이름과 게시글까지** 번역
대상이라, 로케일 문자열 파일이 아니라 DB 스키마와 번역 워커로 풀었음.</sub>
...
<picture>
  ...
  <img alt="인기 페이지 조회수 — 로케일별" src="docs/images/top-pages-light.svg">
</picture>
...
<picture>
  ...
  <img alt="활성 사용자 상위 지역" src="docs/images/regions-light.svg">
</picture>

번역을 로케일 문자열 파일이 아니라 DB 스키마와 백그라운드 워커로 만든 판단
([2번 항목](#2-3개국어를-번역-파일이-아니라-파이프라인으로))이 값을 한 자리임.
...
```mermaid
stateDiagram-v2
    [*] --> Push: main push 하나가 유일한 진입점
    Push --> CI: pre-push hook = CI와 같은 preflight
    CI --> Standby: CI를 통과한 commit SHA만 배포
    Standby --> Verify: 비활성 색상에서 먼저 검증
    Verify --> Switch: Caddy reload
    Verify --> Rollback: 검증 실패
    Switch --> Live
    Switch --> Rollback: 전환 실패 → 이전 라우트·상태 복원
    Live --> Live: 10초 주기 health (약 30초 3회 연속 실패까지 허용)
    Live --> Failover: standby가 직접 health를 통과할 때만
    Live --> Restart: standby가 없거나 불량이면 기존 컨테이너 재시작
    Failover --> Live
    Restart --> Live
    Rollback --> [*]
```
...
**모바일** Android (Kotlin) · iOS (Swift)
**인프라** 자택 Linux 서버 · Docker Compose · Caddy · systemd · GitHub Actions

## 한계와 주의

- **이 저장소에는 실행 가능한 것이 없음.** 코드·데이터베이스·수집 데이터는 공개하지 않고, 구조와
  기술적 판단만 문서로 정리했음. 코드 열람이 필요하면 말씀해 주기 바람.
- 서비스의 콘텐츠 데이터와 이미지 자산은 직접 수집·정리한 것이라 공개 대상이 아님.
- 위 수치(커밋·코드 줄 수·마이그레이션 개수)는 비공개 저장소 기준이며, 계속 늘어남.
  README의 숫자는 2026-09-06 기준임.
- 자택 서버 한 대로 운영하므로 가용성은 관리형 클라우드와 같은 수준이 아님. 그래서 무중단
  배포보다 **자동 복구**에 더 공을 들였음.
- 그래프의 수치는 위 표와 같은 Google Analytics 구간(2026-05-10 ~ 08-30)에서 나온 것이고, 원본
  리포트는 공개하지 않음.
